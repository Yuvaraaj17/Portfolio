  ...
  return (
    <div className='w-full h-full flex items-center py-7' >
        <div ref={parentRef} className='flex justify-center w-full bg-zinc-950 h-[450px] items-center relative'>
            ...
            <div className='opacity-0 absolute scale-[0.3] h-44 w-72 transiiton-all duration-50 ease-out -translate-x-[400px]' ref={emptyLeft}></div>
            <div className='opacity-0 absolute scale-[0.3] h-44 w-72 transiiton-all duration-50 ease-out translate-x-[400px]' ref={emptyRight}></div>
            Focus area
            <button onClick={nextCard} className='absolute bg-pink-500 rounded-sm shadow-md px-2 py-1 left-[250px] flex'>click</button>
            <button onClick={prevCard} className='absolute bg-pink-500 rounded-sm shadow-md px-2 py-1 right-[250px] flex'>click</button>
        </div>
    </div>
  )
}
